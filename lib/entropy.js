...
//dump("Entropy bytes", edlen);
	...
	md5.md5_init();
	for (i = 0; i < edlen; i += 2) {
	    md5.md5_update(entropyData[i]);
	}
	md5.md5_finish();
    	for (i = 0; i < 16; i++) {
	    k[i] = md5.get_digestBit(i);
	}
...
//dump("keyFromEntropy", byteArrayToHex(k));
	...
	exports.addEntropyTime = addEntropyTime;
	exports.keyFromEntropy = keyFromEntropy;
